...
export function send_L_change_request_x_target_L_server(
    neuron_L_el: HTMLElement, 
    neuron_L_id: string, 
    neuron_shell: HTMLElement
){
    let neuron00s_L_ram_X_id = get_L_neuron00s_id00s_x_ram();
    ...
    let controller_f = function (response_obj:any) {
        if (response_obj.status == "success") {
            delete_L_neuron_L_save_flag(neuron_shell, neuron_L_id);
