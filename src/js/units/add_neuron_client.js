...
import {transformate_obj_to_html} from "./tranformate_obj_to_html";

export function add_neuron_client(response_server_id, content, contenteditable, add_ram_boolen, is_format){
   let neuron_content = "";
   ...
   if(is_format == true){
      let html_divs = post_format(new_neuron_el.innerText);
      new_neuron_el.innerHTML =  html_divs.innerHTML;
      functions.search_format_function(new_neuron_el, []);
      put_validation_events_to_neuron(new_neuron_el);
      if(add_ram_boolen == false){
   ...
   let response = {shell: neuron_shell, neuron_el: new_neuron_el};
   return response;
}
