...
import { convert_L_unixtime_L_to_formate_L_usual } from "../units/convert_L_unixtime_L_to_formate_L_usual";
import { focus_L_neuron_L_target_L_tab } from "../units/focus_L_neuron_L_target_L_tab";
import { get_L_tree_L_string } from "../units/get_L_tree_L_string";
import { send_data_ajax } from "../units/send_data_ajax";
import { class_L_neuron } from "./class_L_neuron";
import { class_formate_L_neuron } from "./class_formate_L_neuron";
import { class_generator_L_tree_L_html } from "./class_generator_L_tree_L_html";
...
type neuron_L_feature00s = {
    ...
    tree_L_string: string,
    count_L_string_L_og00s: number,
    is_saved: boolean,
...
class class_L_unit_L_neuron_X_condense {
    ...
    constructor(options:options){
        ...
        if(this.tab_L_unit_X_name == "ram"){
            this.tagbrain_L_unit00s_collection_L_path = "ram_L_unit00s";
        } else if(this.tab_L_unit_X_name == "search"){
            this.tagbrain_L_unit00s_collection_L_path = "unit00s_L_search";
            this.button_L_replace =  '<svg class="linear_icon_L_target icon_size_middle">'
                + '<use xlink:href="#sprite_L_search_icons_x_replace">'
                + '</use>'
            + '</svg>'
        } else if(this.tab_L_unit_X_name == "activation"){ 
            this.tagbrain_L_unit00s_collection_L_path = "neuron00s_L_activated";
        }
        ...
        if(this.unit_L_description_L_short == ""){//empty
            ...
            if(Array.isArray(this.unit_L_description_L_short)){//it is array
                let options: any = {
                    action: "neuron_x_tab_L_usual",
                    tree: this.unit_L_description_L_short,
                    depth_factor: 2
                }
                let generator_L_html: any = new class_generator_L_tree_L_html(
                    options
                );
                let html = generator_L_html.generator_L_html();
                this.unit_L_description_L_short = html;
            } else {//it is string
            }
        }

        this.unit_L_time = convert_L_unixtime_L_to_formate_L_usual(
            options.unit_L_time
        )
    ...
    add_L_unit(){
        ...
        //create_L_unit
        let neuron_L_feature00s = this.collect_L_neuron_L_current_X_features();
        neuron_L_feature00s["button_L_replace"] = this.button_L_replace;
        neuron_L_feature00s["tab_L_unit_X_name"] = this.tab_L_unit_X_name;
        let html = get_compress_html_set(neuron_L_feature00s);
        this.neuron00s_L_select_L_target_L_text();

        //add_unit
        if(this.tab_L_unit_X_name == "ram"){//to_start
            this.output_container_L_element.insertBefore(this.unit_L_element, this.output_container_L_element.firstChild);
        } else {//to_end
            this.output_container_L_element.appendChild(this.unit_L_element );
        }
        this.unit_L_element.className = "unit_L_element"
        this.unit_L_element.innerHTML = html;

        this.push_L_data_L_target_L_tagbrain_obj();
    ...
    set_L_listner_L_field_L_replace_X_change(){
        let unit_L_el = this.unit_L_element;
        let content_L_replacer = this.field_L_replacer_L_value;
        this.field_L_replacer.addEventListener("keyup", (e:any)=>{
            let collections_L_subject_s = unit_L_el.getElementsByTagName("replacer");
            if(collections_L_subject_s.length > 0){
                for(let i = 0; i < collections_L_subject_s.length; i++){
                    collections_L_subject_s[i].innerHTML = content_L_replacer;
                }
            }
        });
    }
    set_L_listner_L_target_L_unit_link(){
        let link_element = this.unit_L_element.querySelector(".link_part") as Element;
        link_element.addEventListener("click", (e:any)=>{
            if(!window["tagbrain_graph"]["neuron00s_obj00s"].hasOwnProperty(this.unit_L_neuron_L_id)){//load_L_neuron
                    ...
                    if (response_obj.status == "success") {
                        for (let i = 0; i < response_obj.data.length; i++) {
                            ...
                            if(i == response_obj.data.length - 1){
                                focus_L_neuron_L_target_L_tab(obj.neuron_id, "neurons");
                            }
                        }
                    } else {
                ...
                focus_L_neuron_L_target_L_tab(this.unit_L_neuron_L_id, tab_L_neuron);
            }
        })
    }
    set_L_listner_L_target_L_unit_L_remove_button(){
        let remove_button_L_element = this.unit_L_element.querySelector(".search_row_delete") as Element;
        remove_button_L_element.addEventListener("click", (e:any)=>{
            this.remove_L_unit_obj_X_el(this.unit_L_neuron_L_id);
        })
    }
    collect_L_neuron_L_current_X_features(){
        ...
        if (this.unit_L_neuron_L_is_special == false) {//neurons usual html
            if(window["tagbrain_graph"]["neuron00s_obj00s"].hasOwnProperty(this.unit_L_neuron_L_id)){//neuron_L_exist_L_in_client
                let neuron_L_obj = window["tagbrain_graph"]["neuron00s_obj00s"][this.unit_L_neuron_L_id];
                let class_neuron = new class_formate_L_neuron(
                    this.unit_L_neuron_L_id, 
                    neuron_L_obj.neuron_el,
                    neuron_L_obj.neuron_shell,
                );
                let tree_L_prepared = class_neuron.tagbrain_graph_L_neuron_L_tree; //for line neuron shape #edit #add
                let obj_L_tree_L_string = get_L_tree_L_string(tree_L_prepared);

                Object.assign(data, {
                    description_L_short: this.unit_L_description_L_short,
                    tree_L_string: obj_L_tree_L_string.string,
                    count_L_string_L_og00s: obj_L_tree_L_string.count,
                    is_saved: true,
                });
