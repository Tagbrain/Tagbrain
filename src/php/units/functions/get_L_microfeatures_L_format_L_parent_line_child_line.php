...
trait get_L_microfeatures_L_format_L_parent_line_child_line {
    use load_neuron_tree;
    protected function get_L_child_L_from_current($outgrowths, $parent_pos){
    ...
    protected function get_L_microfeatures_L_format_L_parent_line_child_line($neuron_path, $neuron_id, $body_regexp){
        $outgrowths = $this->load_neuron_tree($neuron_path, false);
        $microfeature00s_L_obj = array(
            "neuron_id" => $neuron_id,
            "tree_L_microfeature00s" => array(),
            "key_rows" => array(),
            "synapse_L_main" => $outgrowths[0]["content"],
            "count" => 0
        );
        $row_num = 0;
